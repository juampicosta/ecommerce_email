...
} from '../rabbit/cartService'
import { ISession } from 'src/token/token'
...
export async function decrementArticle(
  userId: string,
  body: AddArticleRequest
): Promise<ICart> {
  try {
      ...
      .then((cart) => {
        async.map(
          ...
          (article: ICartArticle, callback) => {
            ...
            restClient
              .get<any>('/v1/articles/' + article.articleId, {
            ...
            cart.articles
              .map((article) => {
                return {
                  article: article,
                  result: results.find(
                    (element) => element._id == article.articleId
                  )
                }
              })
              .forEach((element) => {
 ...
 * @apiUse OtherErrors
 */
export function placeOrder(user: ISession): Promise<void> {
  const { id: userId, email, login } = user
  return new Promise((resolve, reject) => {
    currentCart(userId)
      .then((cart) => {
        if (cart.articles.length == 0) {
          reject(error.newError(400, 'No posee items'))
        }
        cart.enabled = false
        // Save the Cart
        cart.save(function (err: any) {
          if (err) return reject(err)
          sendPlaceOrder(cart)
          sendEmailNotification(email, login)
          resolve()
        })
      })
      .catch((err) => reject(err))
