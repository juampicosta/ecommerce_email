...
export function init() {
  ...
  const cart2 = new RabbitTopicConsumer(
    'topic_cart',
    'sell_flow',
    'order_placed'
  )
 ...
 */
export async function sendArticleValidation(
  cartId: string,
  articleId: string
...
/**
 * @api {direct} notifications/send-email Enviar Email de Checkout
 * @apiGroup RabbitMQ POST
 *
 * @apiDescription Cart enviá un mensaje de email cuando se realiza checkout.
 *
 * @apiExample {json} Mensaje
 *     {
 *        "type": "send-email",
 ...
 *         "message": {
 *             "type": "send-email",
 *             "to_email": "juampicosta81@yopmail.com",
 *             "subject": "🧪 Prueba desde el carrito al hacer checkout!",
 *             "template_id": "68a5dfaafe1eb1ed05b0ac62",
 *             "variables": [
 *                 { "name": "nombre_usuario", "value": "Juan" },
 *                 { "name": "page_title", "value": "rabbit" }
 *             ]
 *        }
 *     }
 */
export async function sendEmailNotification(
  ...
  const message: IRabbitMessage = {
    ...
    message: {
      type: 'send-email',
      to_email,
      subject: 'Felicitaciones! Tu orden ha sido procesada 🛒',
      template_id: conf.placeOrderEmailTemplateId,
      variables: [
        { name: 'nombre_usuario', value: login },
        { name: 'page_title', value: 'rabbit' }
      ]
    }
  }
